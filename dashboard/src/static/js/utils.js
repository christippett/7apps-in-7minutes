...
export class AppBuilder {
    ...
    this.notificationService.addSubscription("build", (message) => {
      ...
      if (status === "started") {
        ...
        let messages = [
          {
            text: [
              "<span class='is-fancy'>Done!</span> That's all seven apps updated in just ",
              "<strong>2 minutes and 32 seconds</strong>.",
            ].join(),
            style: "danger",
          },
          {
            text: [
              "A special thanks to my wife for her considered input into which ",
              "emojis to use for these messages (and her patience as I spent many ",
              "a late night over-engineering this thing).",
            ].join(),
            style: "warning",
          },
  ...
  async deploy() {
    ...
    if (!resp.ok && resp.status != 409) {
      ...
      let messages = [
        "<p>Opps, it seems a another deployment is already in progress. Give it a minute or two (or seven) and try again.</p>",
        "<p>In the meantime, let's check out what's happening with the current deployment.</p>",
        logging_comment,
      ];
      this.tooltip.add({ text: messages.join(), style: "danger" });
    } else {
      let messages = [
        [
          `<p><span class="is-fancy">Woohoo!</span> You just triggered a new <strong>Cloud Build</strong> job!</p>`,
          `<p>Keep an eye out for the version <span class="is-family-monospace">${data.version}</span>, that's yours!</p>`,
        ].join(),
        { text: logging_comment, style: "warning" },
      ];
      setTimeout(() => this.tooltip.queue({ messages, style: "success" }), 1500);
    }
    this.activeBuild = data;
    this.isLoading = false;
    this.timeline.startCountdown();
  }
}
